...
```bash
antonio.martin@MacBook-Pro ~ % docker rm -f $(docker ps -a -q)
bek-i-search: docker_
```

## SETUP ARTIFACTORY

https://artifactory.mpi-internal.com/ui/packages
...
```bash
export ARTIFACTORY_USER=yourusergoeshere@adevinta.com
...
export ARTIFACTORY_CONTEXTURL=https://artifactory.mpi-internal.com/artifactory
```
